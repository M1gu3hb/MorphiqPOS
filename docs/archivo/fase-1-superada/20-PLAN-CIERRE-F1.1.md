...
Eso significa que la capa Kysely completa —repositorios, tipos, SQL— **nunca ha ejecutado una consulta.** El reporte 004 lo dice con todas sus letras y advierte lo más probable: _"la primera venta real puede fallar por algo que ninguna puerta ve, y el candidato más probable es un nombre de columna que no coincida."_
...
_Aceptación:_ `pnpm verify` en verde y `main` con 11 migraciones. Si `verify:inventario` falla buscando `if (movimiento.permiteNegativo) {`, es que el merge se hizo mal: la versión de `stock.ts` que queda debe ser la de `carril-b`.
...
_Aceptación:_ `packages/data/src/esquema.ts` contiene `recetas`, y `recetas.ts` y `resetear.ts` dejan de acceder por SQL crudo.
...
_Aceptación:_ las 11 migraciones registradas, hashes idénticos, cero drift entre archivos y base.
...
_Aceptación:_ tras correrlo, `select count(*) from credenciales_pin` devuelve ≥1 y la consola muestra un código de 6 dígitos.
...
_Aceptación:_ el dueño cambia el PIN de un cajero desde la UI y el cajero entra con el nuevo.
...
_Aceptación:_ el dueño genera un código desde la UI, lo escribe en `/enrolar` de otra pestaña y esa terminal queda enrolada.

**C-07 · ★ Entrar de verdad ★**
Ejecutar: bootstrap → `/enrolar` con el código → `/entrar` con el PIN → llegar a `/venta` con sesión.
_Aceptación:_ la sesión existe, `/api/venta/estado` devuelve 200 y no 401.
...
_Aceptación, y es la que cierra el hito:_
...
_Aceptación:_ C-08 pasa dos veces seguidas, con datos distintos.
...
_Aceptación:_ se cierra una caja desde la UI y el saldo esperado cuadra con `apertura + entradas − salidas`.

**C-11 · Pago mixto en la interfaz**
`repartirPagos` acepta cinco renglones; el diálogo manda uno.
_Aceptación:_ un cobro con efectivo + tarjeta genera **dos filas** en `pagos` y las sumas cuadran al centavo.

**C-12 · El impuesto sale de la configuración**
Hoy está fijo en 1600 puntos base.
_Aceptación:_ cambiar el IVA en `/configuracion` cambia el total de la siguiente cotización.
...
_Aceptación:_ 20 intentos seguidos de PIN desde la misma IP quedan bloqueados; una petición sin `Origin` propio se rechaza.
...
_Aceptación:_ después de correrlo, `auditoria` tiene la fila del reseteo, y las órdenes de atrezzo con folio 1 y 2 ya no están.
...
_Aceptación:_ con paquete `tienda`, un comando de recetas devuelve **403 PAQUETE_NO_INCLUYE**, y la prueba lo verifica con un comando **real**, no con uno de juguete definido dentro del test.
...
_Aceptación:_ el build pasa en Vercel y la URL de `.vercel.app` abre `/entrar`.
...
> _Aceptación:_ el dominio aparece en Vercel esperando DNS, y el reporte trae los registros copiables.
...
_Aceptación:_ una venta hecha **desde la URL pública** aparece en `auditoria`.
...
_Aceptación:_ `pnpm test:e2e` en verde contra un entorno con base real.
...
_Aceptación:_ cero BLOCKERS, y el acta dice qué se ejecutó de verdad y qué no.
