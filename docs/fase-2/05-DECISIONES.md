...
**Lo que NO cambia:** las migraciones se escriben y **no se aplican** a `wyqmzhliurwyxuyxznpb`. Las pruebas unitarias no necesitan Postgres (`vitest.config.ts` excluye las de integración y ninguna unitaria abre conexión), así que la puerta de calidad funciona igual.
