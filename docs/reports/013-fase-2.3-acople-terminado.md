# 013 · Fase 2.3 · ACOPLE TERMINADO · la puerta en 0, contra el despliegue

**Agente:** Claude Code (Opus 5) · **Rama:** `fase-2` · **Fecha:** 17 de septiembre de 2026
**Alcance:** A3 a A7 de `docs/fase-2/F2.3-REGLAS-DE-ACOPLE.md`
**Continúa:** `docs/reports/012-claude-code-fase-2.3-acople.md`, que cerró con A3 bloqueada
...
correr nunca**. Ahora va detrás.

> **CORRECCIÓN (2026-09-17, etapa E3).** Este párrafo decía además que `verify:fase2`
> «también la lleva ahora». **Era falso**: `verify:fase2` terminaba en
> `verify:cobertura` y no llevaba ni `test:integracion`, ni `verify:esquema`, ni
> `verify:rls`, ni `verify:acople`. Se corrigió de la única forma que no deja dos
> verdades: **`verify:fase2` ya no existe.** Hay UNA cadena, `pnpm verify`, y la puerta
> de la fase va dentro.
