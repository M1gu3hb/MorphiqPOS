 ...
 *
 * Portado de `historico/restaurante/src/components/common/BrandedBackground.jsx`
 * sin cambios: mismas clases, mismo degradado, mismo `clampOpacity`.
 */
export default function BrandedBackground() {
  ...
  return (
    <div
      aria-hidden="true"
      className="no-print fixed inset-0 pointer-events-none overflow-hidden -z-0"
    >
...
function clampOpacity(v: number | string | null | undefined): number {
  // `Number` y no `parseFloat`: el lint prohíbe `parseFloat` en todo el
  // repositorio porque reintroduce el punto flotante en los importes (R15).
  // Aquí es una opacidad, no dinero, y para un valor válido las dos funciones
  // dan lo mismo; para basura, `Number` da NaN y cae al 0.12 de siempre.
