 ...
 * cualquier etiqueta que haya que leer, y moverla de columna es un toque que
 * además cambia lo que ve todo el mundo.
 *
 * ── Azul, ámbar y verde; el rojo NO es de aquí ───────────────────────────
 * `primary` es lo que espera, `warning` lo que está en curso, `success` lo
 * confirmado. El rojo queda para lo que está MAL —la alergia—: si el rojo
 * significa «apúrate», el día que signifique «alguien se puede morir» ya nadie
 * lo mira. Y el color nunca va solo: cada columna trae palabra, conteo y emoji.
 *
 * ── El reloj de la tarjeta no tiene umbral ni color ──────────────────────
 * Es el hueco de F-315: sin saber que la arrachera tarda 14 minutos y el
 * pescado 22, «hace 9 minutos» no significa nada, y pintar de ámbar a los diez
 * enseñaría a la cocina a ignorar el ámbar.
 *
 * ── El refresco no mueve nada debajo del dedo ────────────────────────────
 * Se relee cada 2 s, pero dentro de cada columna el orden es la fecha de
 * creación ASCENDENTE: lo nuevo se añade al final y nada salta de sitio. El
 * toque pinta primero y confirma después, y mientras un toque viaja no se
 * relee, para que una respuesta vieja no devuelva la tarjeta a su columna
 * anterior delante del cocinero.
 *
 * ── Lo que NO va aquí ────────────────────────────────────────────────────
 * Precios, totales, propinas, nombres de comensales: ningún dato de dinero. Y
 * no es que la pantalla no los pinte: el servidor no los manda, campo a campo.
 *
 * ── Alcance recortado, para caber en un solo archivo ─────────────────────
 * Fuera quedan las estaciones enteras —filtro, pantalla de bloqueo del
 * cocinero sin estación y vista agrupada por mesa—, porque las tres necesitan
 * resolver la estación desde la sesión del servidor y la página es de una
 * línea a propósito. Fuera quedan también el reloj absoluto y el aviso sonoro
 * del encabezado, que pide su propio permiso de audio. Lo demás está entero.
 */
...
const AVISO = 'rounded-md border border-destructive bg-destructive/15 p-2 text-sm';
 ...
 * único sitio donde se decide cómo se llama un campo es `puente/mapa.ts`, y
 * renombrarlos de paso convierte cada cambio del mapa en un fallo silencioso.
 */
...
export function deLaColumna(
  comandas: readonly ComandaDeCocina[],
  estado: string,
): readonly ComandaDeCocina[] {
  return comandas
 ...
 * tarjeta dos veces: el lector de pantalla las leería dos veces y cada «Marcar
 * listo» existiría dos veces en el DOM.
 */
...
export function Cocina({ filasIniciales }: CocinaProps) {
  ...
  // `ahora` nace en 0 y no en Date.now() para que el servidor y la primera
  // pintura del navegador digan LO MISMO: un reloj sembrado en el servidor es
  ...
  const avanzar = useCallback(async (comanda: ComandaDeCocina, destino: EstadoDestino) => {
    ...
    enVuelo.current += 1;
    try {
      // `transicionar_pedido` (E6-6) rechaza el retroceso por sí solo: dos
      // pantallas de cocina abiertas a la vez no se pisan.
      const ruta = '/api/restaurante/transicionar-pedido';
      await invocarComando(ruta, { comandaId: comanda.id, estado: destino });
    } catch (fallo: unknown) {
  ...
  // Esqueletos con la forma de las tarjetas: la pantalla no salta al cargar y el
  // ojo ya sabe en qué columna va a mirar.
  if (comandas === null) {
    return (
  ...
  return (
    <div className="flex min-h-dvh flex-col gap-4 bg-background p-4 text-foreground">
      ...
      {comandas.length === 0 ? (
        // El único vacío de toda la aplicación que es una BUENA noticia, y se ve
        // como tal: el tipo más grande de la pantalla, y sin una sola disculpa.
        <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-lg bg-success/15 p-8 text-center">
          ...
          <p className="text-muted-foreground">
            La cocina está al día. Lo que se envíe desde el salón aparece aquí solo, sin recargar.
          </p>
        </div>
      ) : esTelefono ? (
...
function Columna({ col, filas, ahora, onAvanzar }: ColumnaProps) {
  const titulado = `columna-${col.estado}`;
  return (
    <section aria-labelledby={titulado} className={`${SECCION} ${col.tinte}`}>
      <h2 id={titulado} className={`${BANDA} ${col.banda}`}>
        <span>
      ...
      {filas.map((comanda) => {
        ...
        const alergias = comanda.notas_alergias;
        return (
          <article key={comanda.id} className={TARJETA}>
            ...
            {alergias !== null && alergias !== '' && (
              <p className={ALERGIA}>
                <span aria-hidden>⚠️ </span>ALERGIA: {alergias}
              </p>
            )}
            <Button
              type="button"
