...
} from '@/utils/ingredienteMatcher';
import {
  normalizarNombreProducto,
  buscarProductoPorNombre,
  buscarCategoriaPorNombre,
} from '@/utils/productoMatcher';
import {
  canonicalUnidad,
  unidadBaseDe,
...
export function validarInventario(rows, { ingredientes }) {
  ...
  for (const r of rows || []) {
    ...
    const unidad = String(r.unidad_base || 'g')
      .trim()
      .toLowerCase();
    ...
    if (!nombreRaw) {
      ...
      out.push({
        status: 'error',
        messages: ['Falta el nombre del ingrediente.'],
        parsed: null,
        line: r.__line,
      });
      continue;
    ...
    if (!UNIDADES_BASE_VALIDAS.includes(unidad)) {
      ...
      out.push({
        status: 'error',
        messages: [`Unidad base inválida "${unidad}". Debe ser g, ml o pieza.`],
        parsed: null,
        line: r.__line,
      });
      continue;
    ...
    if (enVuelo.has(norm)) {
      ...
      out.push({
        status: 'error',
        messages: ['Este ingrediente está repetido en el mismo archivo.'],
        parsed: null,
        line: r.__line,
      });
      continue;
    ...
    if (existente) {
      if (existente.activo === false) {
        inactivos++;
        messages.push(
          `"${existente.nombre}" existe pero está DESACTIVADO. Reactívalo manualmente desde Inventario antes de importar, o cambia el nombre.`,
        );
        out.push({
          status: 'advertencia',
          messages,
          parsed: { ...parsed, _existingId: existente.id, _isInactive: true },
          line: r.__line,
        });
        continue;
      }
      ...
      actualizar++;
      messages.push(
        `Se actualizarán datos maestros de "${existente.nombre}" (no se tocará stock automáticamente).`,
      );
      if (existente.unidad_base && existente.unidad_base !== unidad) {
        messages.push(
          `⚠ La unidad base del archivo ("${unidad}") no coincide con la actual ("${existente.unidad_base}"). Se conservará la actual.`,
        );
      }
      out.push({
        status: 'actualizar',
        ...
        parsed: {
          ...parsed,
          _existingId: existente.id,
          _existingStock: existente.stock_actual || 0,
        },
        line: r.__line,
      });
    } else {
...
export function validarProductos(rows, { productos, categorias }) {
  ...
  for (const r of rows || []) {
    ...
    const area = String(r.area_preparacion || '')
      .trim()
      .toLowerCase();
    ...
    if (!(precio > 0)) {
      ...
      out.push({
        status: 'error',
        messages: ['Precio de venta inválido (debe ser mayor a 0).'],
        parsed: null,
        line: r.__line,
      });
      continue;
    ...
    if (enVuelo.has(norm)) {
      ...
      out.push({
        status: 'error',
        messages: ['Producto repetido en el mismo archivo.'],
        parsed: null,
        line: r.__line,
      });
      continue;
    ...
    if (existente) {
      ...
      out.push({
        status: 'actualizar',
        messages,
        parsed: { ...parsed, _existingId: existente.id },
        line: r.__line,
      });
    } else {
      nuevas++;
      if (cat) messages.push(`Categoría "${cat.nombre}" encontrada.`);
        ...
        messages.push(`Categoría "${categoria}" no existe — se creará al confirmar.`);
      out.push({ status: 'nueva', messages, parsed, line: r.__line });
    }
  }
  ...
  return {
    ...
    resumen: {
      ...
      errores,
      duplicadasArchivo,
      categoriasFaltantes: Array.from(categoriasFaltantes),
    },
  };
}
...
export function validarRecetas(rows, { ingredientes, productos }) {
  ...
  for (const r of rows || []) {
    ...
    if (!productoNombre || !ingredienteNombre) {
      ...
      out.push({
        status: 'error',
        messages: ['Faltan producto o ingrediente.'],
        parsed: null,
        line: r.__line,
      });
      continue;
    ...
    if (!(cantidad > 0)) {
      ...
      out.push({
        status: 'error',
        messages: ['Cantidad inválida (debe ser mayor a 0).'],
        parsed: null,
        line: r.__line,
      });
      continue;
    ...
    if (!ing || !prod) {
      ...
      msgs.push(
        'Crea los datos faltantes antes de importar recetas, o no se podrá registrar esta línea.',
      );
      errores++;
    ...
    if (!compat.compatible) {
      ...
      out.push({
        status: 'error',
        messages: [
          `Unidad "${unidad}" no es compatible con la base "${baseEsperada}" del ingrediente "${ing.nombre}".`,
        ],
        parsed: null,
        line: r.__line,
      });
    ...
    if (!canon) {
      ...
      out.push({
        status: 'error',
        messages: [
          `Unidad "${unidad}" requiere equivalencia explícita y no se admite en importación de recetas. Usa g, ml, kg, l, pieza.`,
        ],
        parsed: null,
        line: r.__line,
      });
    ...
    if ((ing.stock_actual || 0) < cantidadBase) {
      messages.push(
        `ℹ Stock actual (${ing.stock_actual || 0} ${ing.unidad_base}) menor al uso de la receta (${cantidadBase} ${baseEsperada}). Es una ficha técnica, no bloquea.`,
      );
    }
...
export function validarProveedores(rows, { proveedores }) {
  ...
  for (const r of rows || []) {
    ...
    if (!nombre) {
      ...
      out.push({
        status: 'error',
        messages: ['Falta el nombre del proveedor.'],
        parsed: null,
        line: r.__line,
      });
      continue;
    ...
    if (enVuelo.has(norm)) {
      ...
      out.push({
        status: 'error',
        messages: ['Proveedor repetido en el mismo archivo.'],
        parsed: null,
        line: r.__line,
      });
      continue;
  ...
  return {
    rows: out,
    resumen: { total: out.length, nuevas, actualizar, errores, duplicadasArchivo },
  };
}
...
export function validarGastos(rows) {
  ...
  for (const r of rows || []) {
    ...
    const metodo = String(r.metodo_pago || 'efectivo')
      .trim()
      .toLowerCase();
    ...
    if (catFinal !== categoria)
      messages.push(`Categoría "${categoria}" no reconocida. Se usará "otro".`);
