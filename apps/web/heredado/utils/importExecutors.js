 ...
 */
export async function ejecutarImportInventario(
  previewRows,
  { ajustarStock = false, posUser, fechaIso },
) {
  const reporte = {
    creados: 0,
    actualizados: 0,
      ...
      if (p._existingId) {
        ...
        if (ajustarStock) {
          ...
          if (delta !== 0) {
            ...
            await api.entidades.Ingrediente.update(p._existingId, { stock_actual: newStock });
        ...
        const created = await api.entidades.Ingrediente.create({
          nombre: p.nombre,
          unidad_base: p.unidad_base,
  ...
  for (const [productoId, lineas] of porProducto.entries()) {
    ...
    try {
      ...
      for (const l of lineas) {
        ...
        const creada = await api.entidades.RecetaEscandallo.create({
      ...
      if (reemplazarExistente && existentesActivas.length > 0) {
        for (const old of existentesActivas) {
          try {
            await api.entidades.RecetaEscandallo.update(old.id, { activo: false });
            reporte.lineas_inactivadas++;
          } catch (errInact) {
            reporte.errores.push(
              `Aviso: no se pudo inactivar línea previa de "${productoNombre}": ${errInact?.message || ''}`,
            );
          }
        }
      }
      ...
      try {
        const prod = await api.entidades.ProductoTerminado.get(productoId);
        if (prod) {
          const precio = Number(prod.precio_venta) || 0;
          const utilidad = precio - costoTotal;
          const margen = precio > 0 ? (utilidad / precio) * 100 : 0;
          await api.entidades.ProductoTerminado.update(productoId, {
      ...
      for (const idCreado of lineasCreadasIds) {
        try {
          await api.entidades.RecetaEscandallo.delete(idCreado);
        } catch {}
      }
