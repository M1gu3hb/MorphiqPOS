...
async function resolverVentaActiva({ mesaId, ventaIdHint }) {
  ...
  if (mesaId) {
    try {
      mesa = await api.entidades.Mesa.get(mesaId);
    } catch {
      mesa = null;
    }
  }
...
export async function entregarPedidosListosDeMesa({ mesaId, ventaId, pedidoIds }) {
  ...
  const { ventaId: ventaIdResuelto, mesa } = await resolverVentaActiva({
    mesaId,
    ventaIdHint: ventaId,
  });
  ...
  let pedidos = [];
  try {
    pedidos = await api.entidades.PedidoPreparacion.filter({ venta_id: ventaIdResuelto });
  } catch {
    pedidos = [];
  }
  ...
  if (pedidos.length === 0) {
    try {
      const porMesa = await api.entidades.PedidoPreparacion.filter({ mesa_id: mesaId });
  ...
  const updates = await Promise.all(
    candidatos.map(async (p) => {
      try {
        await api.entidades.PedidoPreparacion.update(p.id, {
          estado: 'entregado',
  ...
  resultado.estaciones = [
    ...new Set(exitosos.map((p) => p?.estacion_preparacion_nombre || '').filter(Boolean)),
  ];
  ...
  const restantesActivos = pedidos.filter(
    (p) => p && !idsEntregados.has(p.id) && !['entregado', 'cancelado'].includes(p?.estado),
  );
  ...
  if (restantesActivos.length === 0 && mesa?.estado) {
    ...
    if (!estadosNoTocar.includes(mesa.estado)) {
      try {
        await api.entidades.Mesa.update(mesaId, { estado: 'ocupada' });
