 ...
 */
export async function ensureCategoriaExists(api, nombre, opts = {}) {
  ...
  const todas = await api.entidades.CategoriaProducto.list().catch(() => []);
  ...
  if (existente) {
    ...
    if (existente.activo === false && opts.reactivate !== false) {
      try {
        await api.entidades.CategoriaProducto.update(existente.id, { activo: true });
      } catch {}
  ...
  return await api.entidades.CategoriaProducto.create(payload);
}
