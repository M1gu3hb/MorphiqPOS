...
export function resolverEstacionParaProducto(producto, categorias, estaciones, config) {
  ...
  const estacionesArr = Array.isArray(estaciones)
    ? estaciones.filter((e) => e?.activo !== false)
    : [];
...
export function agruparItemsPorEstacion(items, productosMap, categorias, estaciones, config) {
  ...
  arr.forEach((item) => {
    ...
    const productoBase =
      productosMap && item?.id && productosMap.get ? productosMap.get(item.id) || item : item;
