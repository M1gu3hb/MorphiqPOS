 ...
 */
export function mlPorPorcionEfectivo({
  ml_por_porcion,
...
export function validarProductoVariable(producto = {}) {
  ...
  if (tipo === TIPO_VENTA.VARIABLE_MEDIDA) {
    ...
    if (!UNIDADES_VARIABLE.includes(producto?.unidad_variable))
      errores.push('Unidad de venta inválida.');
    if (toPos(producto?.precio_por_unidad_variable) === 0)
      errores.push('Precio por unidad debe ser > 0.');
    ...
    if (min > 0 && max > 0 && min > max)
      errores.push('Cantidad mínima no puede ser mayor que la máxima.');
  } else if (tipo === TIPO_VENTA.PORCION_CONTENEDOR) {
    if (!producto?.ingrediente_base_id) errores.push('Falta ingrediente base.');
    if (toPos(producto?.capacidad_contenedor_ml) === 0)
      errores.push('Capacidad del contenedor debe ser > 0.');
    if (toPos(producto?.porciones_por_contenedor) === 0)
      errores.push('Porciones por contenedor debe ser > 0.');
