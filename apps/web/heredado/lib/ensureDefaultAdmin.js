...
export async function ensureDefaultAdmin() {
  ...
  inFlight = (async () => {
    try {
      ...
      const all = await api.entidades.UsuarioPOS.list('-created_date', 5);
      if (Array.isArray(all) && all.length > 0) {
        return { created: false, reason: 'users_exist' };
      }
      const created = await api.entidades.UsuarioPOS.create(DEFAULT_ADMIN);
...
export function isUsingDefaultAdminPin(usuarios) {
  ...
  return usuarios.some(
    (u) => u?.rol === 'administrador' && u?.activo !== false && u?.pin === '1234',
  );
}
