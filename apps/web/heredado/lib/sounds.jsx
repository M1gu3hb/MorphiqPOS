...
const isEnabled = () => {
  try {
    ...
    const v =
      localStorage.getItem('mh_sounds_enabled') ?? localStorage.getItem('azecafe_sounds_enabled');
    return v !== 'false';
  } catch {
    return true;
  }
...
const getVolume = () => {
  try {
    const raw =
      localStorage.getItem('mh_sounds_volume') ?? localStorage.getItem('azecafe_sounds_volume');
...
export const setSoundsEnabled = (enabled) => {
  try {
    localStorage.setItem('mh_sounds_enabled', enabled ? 'true' : 'false');
  } catch {}
};
export const setSoundsVolume = (vol) => {
  try {
    localStorage.setItem('mh_sounds_volume', String(vol));
  } catch {}
};
