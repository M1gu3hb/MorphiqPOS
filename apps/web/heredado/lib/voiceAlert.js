...
function writeVoiceCache(map) {
  try {
    localStorage.setItem(KEY_VOICE_CACHE, JSON.stringify(map || {}));
  } catch {}
}
...
export function getPreferredVoiceName() {
  try {
    return localStorage.getItem(KEY_VOICE_MANUAL) || '';
  } catch {
    return '';
  }
}
