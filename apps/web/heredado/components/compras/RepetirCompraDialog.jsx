...
export default function RepetirCompraDialog({ open, onClose, onSelected }) {
  ...
  const comprasFiltered = compras.filter(
    (c) =>
      !q || (c?.proveedor_nombre || '').toLowerCase().includes(q) || (c?.fecha || '').includes(q),
  );
  ...
  const cargarDetalle = async (compraId) => {
    ...
    try {
      const dets = await api.entidades.DetalleCompra.filter({ compra_id: compraId });
  ...
  const pickCompra = async (compra) => {
    ...
    if (!dets) {
      try {
        dets = await api.entidades.DetalleCompra.filter({ compra_id: compra.id });
      } catch {
        dets = [];
      }
    }
  ...
  const eliminarPlantilla = async (p, e) => {
    ...
    try {
      ...
      await api.entidades.PlantillaCompra.update(p.id, { activa: false });
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-2">
                      ...
                      {loadingDetalle !== c.id && Array.isArray(dets) && dets.length > 0 && (
                        <>
                          <p className="text-[11px] font-semibold text-muted-foreground mb-1.5 flex items-center gap-1">
                            <Package className="w-3 h-3" /> {dets.length} ingrediente
                            {dets.length === 1 ? '' : 's'}
                          </p>
                            ...
                            {dets.map((d) => (
                              ...
                              >
                                ...
                                <span className="text-muted-foreground whitespace-nowrap">
                                  {Number(d.cantidad_comprada || 0).toLocaleString()}{' '}
                                  {d.unidad_compra || ''}
                                </span>
          ...
          {tab === 'plantillas' && !loading && plantillas.length === 0 && (
            <div className="text-center py-6">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Al guardar una compra, marca "Guardar como plantilla recurrente".
              </p>
            </div>
          )}
          {tab === 'plantillas' &&
                ...
                <Card key={p.id} className="overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground flex items-center gap-1.5 flex-wrap">
                        ...
                        {p.ultima_fecha_uso && (
                          <span>
                            Última vez:{' '}
                            {format(new Date(p.ultima_fecha_uso), 'd MMM yyyy', { locale: es })}
                          </span>
                        )}
                      </p>
                    </div>
                    ...
                    <Button
                      ...
                      onClick={(e) => eliminarPlantilla(p, e)}
