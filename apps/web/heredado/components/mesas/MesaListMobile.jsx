...
export default function MesaListMobile({ mesas, onEdit, onReorder }) {
  const sorted = [...mesas].sort(
    (a, b) => (a.orden || 0) - (b.orden || 0) || (a.numero || 0) - (b.numero || 0),
  );
      ...
      {sorted.map((m, idx) => (
        <Card key={m.id} className="p-3 flex items-center gap-3">
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(135deg, #f4ead5, #e8dcc0)',
              border: '2px solid #c9b68a',
            }}
          >
            {m.numero}
          </div>
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground">
              {m.zona || 'Interior'} · <span className="capitalize">{m.forma}</span> ·{' '}
              <span className="capitalize">{m.tamano}</span>
            </p>
