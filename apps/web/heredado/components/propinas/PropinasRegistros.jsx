...
export default function PropinasRegistros() {
  ...
  const { data: ventas = [] } = useQuery({
    ...
    queryFn: () => api.entidades.Venta.list('-fecha_cierre', 2000),
    initialData: [],
  });
  ...
  const ventasFiltradas = useMemo(() => {
    ...
    if (meseroId && meseroId !== '__all__')
      res = res.filter((v) => v?.usuario_mesero_id === meseroId);
    return res;
      ...
      <Card className="p-4 bg-white/80 backdrop-blur-sm">
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          ...
          <div className="ml-auto flex gap-2 flex-wrap">
            ...
            >
              <History className="w-3.5 h-3.5 mr-1" /> {verHistorial ? 'Ocultar' : 'Ver'}{' '}
              liquidaciones
            </Button>
            <Button
              size="sm"
                ...
                >
                  <div className="flex-1 min-w-[180px]">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {l.fecha_liquidacion
                        ? format(new Date(l.fecha_liquidacion), 'd MMM yyyy HH:mm', { locale: es })
                        : ''}
        ...
        {ventasFiltradas.length === 0 ? (
          ...
          <div className="divide-y max-h-[60vh] overflow-y-auto">
            {ventasFiltradas.slice(0, 300).map((v) => (
              <div
                key={v.id}
                className="px-4 py-2.5 flex items-center gap-3 flex-wrap text-sm hover:bg-muted/30"
              >
                <div className="flex-1 min-w-[140px]">
                  <p className="font-mono font-bold text-xs">{v.folio}</p>
                  <p className="text-[10px] text-muted-foreground">
                    {v.fecha_cierre
                      ? format(new Date(v.fecha_cierre), 'd MMM HH:mm', { locale: es })
                      : ''}
