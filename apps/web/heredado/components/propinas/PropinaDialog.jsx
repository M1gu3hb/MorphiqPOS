...
export default function PropinaDialog({
  ...
  const confirmar = (tipo) => {
    ...
    if (pctActivo !== null) {
      onConfirm?.({
        propina_monto: propinaCalculada,
        propina_porcentaje: pctActivo,
  ...
  const gridCols =
    porcentajes.length >= 4 ? 'grid-cols-2 sm:grid-cols-4' : `grid-cols-${porcentajes.length}`;
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-md w-[calc(100%-2rem)] max-h-[92vh] overflow-y-auto p-4 sm:p-6 gap-3">
        ...
        <div
          className={`grid gap-2 pt-1 ${allowPendiente ? 'grid-cols-2 sm:grid-cols-3' : 'grid-cols-2'}`}
        >
          <Button
            variant="outline"
          ...
          <Button
            ...
            style={{
              background: puedeAplicar
                ? 'linear-gradient(135deg, hsl(340,82%,52%) 0%, hsl(340,82%,42%) 100%)'
                : undefined,
            }}
          >
