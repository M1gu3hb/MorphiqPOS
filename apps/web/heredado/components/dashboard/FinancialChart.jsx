...
export default function FinancialChart({ ventas = [], gastos = [] }) {
  ...
  const { data, totales } = useMemo(() => {
    ...
    // Build buckets
    const buckets = {};
    const keyOf = (d) => (groupBy === 'month' ? format(d, 'yyyy-MM') : format(d, 'yyyy-MM-dd'));
    const labelOf = (d) =>
      groupBy === 'month' ? format(d, 'MMM', { locale: es }) : format(d, 'd MMM', { locale: es });
    const emptyBucket = (k, lab) => ({
      key: k,
      label: lab,
      ventas: 0,
      costo: 0,
    ...
    while (cursor <= endDate && guard < 400) {
      // guard contra loops infinitos por fechas inválidas
    ...
    ventasFil.forEach((v) => {
      ...
      const utilidad = Number.isFinite(Number(utilidadSnap)) ? N(utilidadSnap) : ventaReal - costo;
  ...
  return (
    <Card
      className="overflow-hidden border-2 bg-card text-card-foreground"
    ...
    >
      ...
      <CardContent>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mb-3">
          ...
          <Mini
            label="Margen"
            value={`${(Number.isFinite(totales.margen) ? totales.margen : 0).toFixed(1)}%`}
            color="#0f172a"
            raw
          />
        </div>
