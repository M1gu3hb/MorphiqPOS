...
const BLOQUES = [
  {
    ...
    fetch: () => api.entidades.Ingrediente.list('-created_date', 5000),
    columns: [
      { key: 'id', label: 'id' },
  ...
  },
  {
    ...
    fetch: () => api.entidades.ProductoTerminado.list('-created_date', 5000),
    columns: [
      { key: 'id', label: 'id' },
      { key: 'nombre', label: 'nombre' },
      ...
      {
        key: 'visible_en_pos',
        label: 'visible_en_pos',
        format: (v) => (v === false ? 'no' : 'sí'),
      },
      { key: 'activo', label: 'activo', format: (v) => (v === false ? 'no' : 'sí') },
    ],
  },
  {
    ...
    custom: async () => {
      ...
      const [recetas, productos] = await Promise.all([
        api.entidades.RecetaEscandallo.list('-created_date', 10000),
  ...
  },
  {
    ...
    fetch: () => api.entidades.Proveedor.list('-created_date', 5000),
    columns: [
      { key: 'id', label: 'id' },
      { key: 'nombre', label: 'nombre' },
  ...
  },
  {
    ...
    fetch: () => api.entidades.CompraInsumo.list('-fecha', 10000),
    columns: [
      { key: 'id', label: 'id' },
  ...
  },
  {
    ...
    fetch: () => api.entidades.GastoOperativo.list('-fecha', 10000),
    columns: [
      { key: 'id', label: 'id' },
      { key: 'fecha', label: 'fecha' },
      { key: 'descripcion', label: 'descripcion' },
  ...
  },
  {
    ...
    fetch: () => api.entidades.PlantillaGasto.filter({ activa: true }),
    columns: [
      { key: 'id', label: 'id' },
      { key: 'nombre', label: 'nombre' },
      { key: 'categoria', label: 'categoria' },
  ...
  },
  {
    ...
    fetch: () => api.entidades.CategoriaProducto.filter({ activo: true }),
    columns: [
      { key: 'id', label: 'id' },
      { key: 'nombre', label: 'nombre' },
      { key: 'descripcion', label: 'descripcion' },
...
export default function ExportarDatos() {
  ...
  const exportarTodo = async () => {
    ...
    toast.success(
      `Respaldo completo: ${ok} archivo(s) descargado(s)${fail ? `, ${fail} falló(aron)` : ''}.`,
    );
  };
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-xs text-muted-foreground">
          Descarga tu información en CSV compatible con Excel y Google Sheets. Exportar nunca
          modifica tus datos — es solo de lectura.
        </p>
      </CardHeader>
