...
import {
  ejecutarImportInventario,
  ejecutarImportProductos,
  ejecutarImportRecetas,
  ejecutarImportProveedores,
  ejecutarImportGastos,
...
export default function ImportarDatosDialog({ open, onClose, tipo }) {
  ...
  const ejecutar = async () => {
    ...
    try {
      ...
      if (tipo === 'inventario') {
        ...
        r = await ejecutarImportProductos(preview.rows, {
          crearCategoriasFaltantes: crearCategorias,
        });
  ...
  const hayFilasValidas =
    preview && (preview.rows || []).some((r) => r.status !== 'error' && r.parsed);
      ...
      <DialogContent className="max-w-3xl max-h-[92vh] overflow-hidden flex flex-col p-0">
        <DialogHeader className="px-5 pt-4 pb-3 border-b">
          ...
          <DialogDescription className="text-xs">
            Paso {paso} de 3 —{' '}
            {paso === 1 ? 'cargar archivo' : paso === 2 ? 'vista previa' : 'reporte final'}. Nada se
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <Label className="text-sm font-medium">
                        Ajustar stock al valor del archivo
                      </Label>
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        Si está apagado, solo se actualizan datos maestros (costo, mínimos) sin
                        tocar stock. Si está encendido, el stock se ajusta y se crea
                        MovimientoInventario por cada cambio.
                      </p>
                    </div>
                ...
                <div className="p-3 rounded-lg border bg-muted/30 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        Si está apagado, los productos con categoría inexistente se crearán sin
                        categoría.
                      </p>
                    </div>
                ...
                <div className="p-3 rounded-lg border bg-muted/30 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        Si el producto ya tiene receta y esta opción está apagada, se OMITIRÁ para
                        evitar borrar tu trabajo.
                      </p>
                    </div>
          ...
          {/* PASO 3: reporte */}
          {paso === 3 && reporte && <ReporteFinal reporte={reporte} tipo={tipo} />}
        </div>
        ...
        <DialogFooter className="px-5 py-3 border-t flex-row gap-2 justify-end">
              ...
              <Button
                ...
                title={
                  bloqueadoPorErroresCriticos
                    ? 'Hay errores en el archivo. Corrige el CSV y vuelve a cargarlo.'
                    : ''
                }
              >
                {ejecutando ? (
          ...
          {paso === 3 && <Button onClick={handleClose}>Cerrar</Button>}
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
        ...
        (resumen.ingredientesFaltantes?.length > 0 || resumen.productosFaltantes?.length > 0) && (
          <div className="p-3 rounded-lg bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800/60 text-xs space-y-2">
            {resumen.ingredientesFaltantes?.length > 0 && (
              <div>
                <p className="font-medium mb-1">
                  Ingredientes faltantes ({resumen.ingredientesFaltantes.length}):
                </p>
                <div className="flex flex-wrap gap-1">
                  ...
                  {resumen.ingredientesFaltantes.length > 30 && (
            ...
            {resumen.productosFaltantes?.length > 0 && (
              <div>
                <p className="font-medium mb-1">
                  Productos faltantes ({resumen.productosFaltantes.length}):
                </p>
                <div className="flex flex-wrap gap-1">
            ...
            <p className="text-[10px] mt-1">
              Crea estos datos primero (inventario / productos) y vuelve a subir el archivo.
            </p>
          </div>
        )}
...
function FilaPreview({ fila, tipo }) {
  ...
  const titulo =
    fila.parsed?.nombre ||
    fila.parsed?.producto_nombre ||
    fila.parsed?.descripcion ||
    `Línea ${fila.line}`;
  return (
...
function ReporteFinal({ reporte, tipo }) {
  ...
  if ('omitidos_inactivos' in reporte)
    items.push({ k: 'Omitidos (inactivos)', v: reporte.omitidos_inactivos });
  if ('categorias_creadas' in reporte)
    items.push({ k: 'Categorías creadas', v: reporte.categorias_creadas });
  if ('productos_actualizados' in reporte)
    items.push({ k: 'Productos actualizados', v: reporte.productos_actualizados });
  if ('lineas_creadas' in reporte) items.push({ k: 'Líneas creadas', v: reporte.lineas_creadas });
  if ('productos_omitidos' in reporte)
    items.push({ k: 'Productos omitidos', v: reporte.productos_omitidos });
