...
export default function MovimientosPanel({ movimientos = [] }) {
  ...
  const ingredientesList = useMemo(() => {
    ...
    movimientos.forEach((m) => {
      if (m.ingrediente_id) map[m.ingrediente_id] = m.ingrediente_nombre;
    });
  ...
  const range = useMemo(() => {
    ...
    if (periodo === 'year')
      return { from: startOfYear(now).getTime(), to: endOfDay(now).getTime() };
    return null;
  }, [periodo]);
          ...
          {grouped.map(([day, items]) => (
            <div key={day}>
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1.5 font-semibold">
                {day ? format(new Date(day), 'EEEE d MMM yyyy', { locale: es }) : 'Sin fecha'} ·{' '}
                {items.length}
              </p>
