 ...
 */
export default function CocinaMesaGroupCard({ pedidos, onIniciar, onListo, onQuitarListo }) {
  const isDark = useIsDark();
  const arr = Array.isArray(pedidos) ? pedidos : [];
  if (arr.length === 0) return null;

  const first = arr[0] || {};
  const mesaLabel = first?.mesa_numero ? `Mesa ${first.mesa_numero}` : 'Mostrador';
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-muted-foreground flex items-center gap-1 leading-tight">
            <Clock className="w-2.5 h-2.5" /> {timeLabel} · {totalItems} item
            {totalItems !== 1 ? 's' : ''}
          </p>
        </div>
