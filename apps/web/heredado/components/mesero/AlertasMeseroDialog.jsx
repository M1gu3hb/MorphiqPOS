...
export default function AlertasMeseroDialog({ open, onClose }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      ensureVoicesLoaded().then(() => {
        try {
          setVoiceInfo(getCurrentVoiceInfo(getAlertConfig().lang || 'es-MX'));
        } catch {}
      });
    }
  }, [open]);
  ...
  const probar = () => {
    ...
    if (cfg.modo === 'voz' || cfg.modo === 'sonido_voz') {
      ...
      if (!isVoiceSupported())
        toast.info('Sonido reproducido. La voz local no está soportada en este navegador.');
    }
  };
      ...
      <DialogContent className="sm:max-w-md w-[calc(100%-2rem)] max-h-[92vh] overflow-y-auto p-4 sm:p-6">
        ...
        <div className="space-y-4">
          <p className="text-xs text-muted-foreground">
            Esta configuración es personal de este dispositivo. Si tu navegador bloquea el audio,
            toca <strong>Activar audio</strong>.
          </p>
          ...
          <div
            className={`flex items-center justify-between gap-3 rounded-xl border p-3 ${!asignOn ? 'opacity-60' : ''}`}
          >
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground truncate">
                {asignOn
                  ? posUser?.nombre
                    ? `Ejemplo: "${posUser.nombre}, mesa 4 solicita cuenta."`
                ...
                <div className="min-w-0">
                  ...
                  {voiceInfo.available ? (
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      Aún no se detecta voz en español. Toca "Elegir mejor voz".
                    </p>
                  )}
                </div>
