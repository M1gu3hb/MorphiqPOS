 ...
 * onUpdateExclusiones(item.id, [nombres]) — lista visible para UI.
 */
export default function ProductoFichaExpandible({
  item,
  onUpdateNotas,
  onUpdateExclusiones,
  onCambiarCantidad,
  ...
  const [exclusiones, setExclusiones] = useState(
    Array.isArray(item?._exclusiones) ? item._exclusiones : [],
  );

  // Sincronizar cambios externos al item (por si lo agregaron de nuevo)
  useEffect(() => {
    setNota(item?._notaUsuario || '');
    setExclusiones(Array.isArray(item?._exclusiones) ? item._exclusiones : []);
  }, [item?.id]);

  // Recetas del producto — solo nombres, sin cantidades ni costos.
  const { data: recetas = [] } = useQuery({
    queryKey: ['mesero_receta_prod', item?.id, open],
    queryFn: () => api.entidades.RecetaEscandallo.filter({ producto_id: item.id }),
    initialData: [],
  ...
  const toggleExclusion = (nombre) => {
    setExclusiones((prev) => {
      ...
      if (typeof onUpdateExclusiones === 'function') onUpdateExclusiones(item.id, next);
  ...
  const onNotaChange = (val) => {
    setNota(val);
    if (typeof onUpdateNotas === 'function')
      onUpdateNotas(item.id, buildNotaFinal(exclusiones, val));
  };
        ...
        {open && (
          ...
          >
            <div className="p-2.5 space-y-2">
              {item?.descripcion && (
                <p className="text-[11px] text-muted-foreground italic">{item.descripcion}</p>
              )}
