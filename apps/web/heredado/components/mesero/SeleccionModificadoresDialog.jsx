...
export default function SeleccionModificadoresDialog({ producto, open, onClose, onConfirm }) {
  ...
  const grupos = useMemo(() => {
    ...
    return arr
      ...
      .map((g) => ({
        ...g,
        opciones: (Array.isArray(g.opciones) ? g.opciones : []).filter(
          (o) => o?.activo !== false && (o?.nombre || '').trim(),
        ),
      }))
  ...
  const handleConfirm = () => {
    ...
    for (const g of grupos) {
      ...
      const tieneSeleccion = g.tipo === 'multiple' ? Array.isArray(v) && v.length > 0 : !!v;
