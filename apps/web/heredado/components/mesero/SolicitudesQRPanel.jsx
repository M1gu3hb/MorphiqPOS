...
export default function SolicitudesQRPanel() {
  ...
  const { data: solicitudesRaw = [] } = useQuery({
    ...
    queryFn: () => api.entidades.SolicitudQR.list('-created_date', 100),
    initialData: [],
  ...
  const activas = useMemo(() => {
    ...
    return (Array.isArray(arr) ? arr : []).filter(
      (s) => s?.estado === 'pendiente' || s?.estado === 'atendida',
    );
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <p className="text-sm text-muted-foreground">
          {activas.length === 0
            ...
            : `${activas.length} solicitud(es) activas`}
          {asign && (
            <span className="ml-1 text-[10px] uppercase tracking-wide text-primary font-bold">
              · Modo asignación
            </span>
          )}
        </p>
