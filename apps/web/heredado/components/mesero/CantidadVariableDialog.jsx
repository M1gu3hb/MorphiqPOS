...
export default function CantidadVariableDialog({ open, producto, onClose, onConfirm }) {
  ...
  useEffect(() => {
    ...
    if (esMedida) {
      ...
      setUnidadElegida(
        UNIDADES_VARIABLE.includes(producto.unidad_variable) ? producto.unidad_variable : 'g',
      );
  ...
  const presets = useMemo(() => {
    if (esMedida)
      return Array.isArray(producto?.presets_variable_qr) ? producto.presets_variable_qr : [];
    if (esPorcion)
      return Array.isArray(producto?.presets_porcion_qr) ? producto.presets_porcion_qr : [];
    return [];
          ...
          <div className="rounded-lg p-3 bg-muted/40 border text-xs space-y-0.5">
            {esMedida && (
              <>
                <p>
                  ...
                  {formatCurrency(Number(producto?.precio_por_unidad_variable) || 0)} por{' '}
                  {producto?.unidad_variable || 'g'}
                </p>
                {(producto?.cantidad_minima_variable > 0 ||
                  producto?.cantidad_maxima_variable > 0) && (
            ...
            {esPorcion && (
              <>
                <p>
                  <strong>Precio:</strong>{' '}
                  {formatCurrency(Number(producto?.precio_por_porcion) || 0)} por{' '}
                ...
                <p className="text-muted-foreground">
                  ...
                  {producto?.nombre_porcion || 'porciones'}
                </p>
              </>
            )}
          </div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-1">
                {presets.map((p, i) => {
                  ...
                  return (
                    ...
                    >
                      {valor}{' '}
                      {esMedida ? producto?.unidad_variable || '' : producto?.nombre_porcion || ''}
                    </button>
                  );
                })}
              </div>
