          ...
          {(lineas || []).map((l, i) => (
            <tr key={i} className="border-b border-dashed">
              ...
              <td className="py-1 text-right">
                {formatCurrency(l.costo_unitario_base_snapshot || 0)}
              </td>
              <td className="py-1 text-right font-medium">
                {formatCurrency(l.costo_linea_calculado || 0)}
              </td>
            </tr>
          ))}
        </tbody>
...
function FichaVariable({ producto, ingrediente, config, fecha }) {
  ...
  if (esMedida) {
    ...
    configRows = [
      ...
      Number(producto.cantidad_minima_variable) > 0 && [
        'Cantidad mínima',
        `${producto.cantidad_minima_variable} ${u}`,
      ],
      Number(producto.cantidad_maxima_variable) > 0 && [
        'Cantidad máxima',
        `${producto.cantidad_maxima_variable} ${u}`,
      ],
      Number(producto.incremento_variable) > 0 && [
        'Incremento',
        `${producto.incremento_variable} ${u}`,
      ],
      Array.isArray(producto.presets_variable_qr) &&
        producto.presets_variable_qr.length > 0 && [
          'Presets QR',
          producto.presets_variable_qr.join(', ') + ' ' + u,
        ],
    ].filter(Boolean);
  } else {
    ...
    configRows = [
      ...
      Number(producto.capacidad_contenedor_ml) > 0 && [
        ...
        `${producto.capacidad_contenedor_ml} ml`,
      ],
      Number(producto.porciones_por_contenedor) > 0 && [
        'Porciones por contenedor',
        String(producto.porciones_por_contenedor),
      ],
      mlPorcion > 0 && ['ml por porción', `${mlPorcion} ml`],
      Array.isArray(producto.presets_porcion_qr) &&
        producto.presets_porcion_qr.length > 0 && [
          'Presets QR',
          producto.presets_porcion_qr.join(', ') + ' ' + np,
        ],
    ].filter(Boolean);
  }
