...
export default function ModificadoresEditor({ value, onChange, disabled = false }) {
  ...
  const removeOpcion = (gIdx, oIdx) => {
    emit(
      grupos.map((g, i) =>
        i === gIdx ? { ...g, opciones: g.opciones.filter((_, j) => j !== oIdx) } : g,
      ),
    );
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {grupos.map((g, gIdx) => {
        ...
        const opcionesActivas = (g.opciones || []).filter(
          (o) => o.activo !== false && normalizeName(o.nombre),
        );
      ...
      <p className="text-[10px] text-muted-foreground italic">
        Estas opciones son informativas para mesero y cocina. No afectan el precio, costo ni
        inventario.
      </p>
    </div>
  );
