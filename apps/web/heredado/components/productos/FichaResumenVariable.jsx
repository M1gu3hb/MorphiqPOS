...
import { TIPO_VENTA, mlPorPorcionEfectivo, unidadBaseDesdeMostrada } from '@/utils/tipoVentaUtils';
    ...
    return (
      <div className="grid grid-cols-3 gap-1 text-center">
        ...
        <div className="rounded-md bg-muted/50 py-1.5">
          ...
          <p
            className={`text-xs font-bold ${resumen.hayCosto ? margenColor(resumen.margen) : 'text-muted-foreground'}`}
          >
  ...
  return (
    <div className="rounded-xl border bg-card p-4 space-y-3">
      ...
      <div className="text-[11px] text-muted-foreground space-y-0.5">
        {resumen.tipo === 'medida' && resumen.min > 0 && (
          <div>
            Mínimo: {resumen.min} {u}
          </div>
        )}
        {resumen.tipo === 'medida' && resumen.max > 0 && (
          <div>
            Máximo: {resumen.max} {u}
          </div>
        )}
        {resumen.tipo === 'medida' && resumen.incremento > 0 && (
          <div>
            Incremento: {resumen.incremento} {u}
          </div>
        )}
        {resumen.tipo === 'porcion' && resumen.mlPorcion > 0 && (
          <div>
            {resumen.mlPorcion} ml / {u}
          </div>
        )}
        {resumen.tipo === 'porcion' && resumen.capacidad > 0 && (
          <div>Capacidad contenedor: {resumen.capacidad} ml</div>
        )}
        {resumen.tipo === 'porcion' && resumen.porciones > 0 && (
          <div>{resumen.porciones} porciones / contenedor</div>
        )}
        {resumen.presets.length > 0 && (
          <div>
            Presets QR: {resumen.presets.join(', ')} {u}
          </div>
        )}
      </div>
