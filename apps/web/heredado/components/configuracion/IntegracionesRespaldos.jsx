...
export default function IntegracionesRespaldos() {
  ...
  const { data: pendingLogs = [] } = useQuery({
    ...
    queryFn: () => api.entidades.IntegrationSyncLog.filter({ status: 'pending_external_sync' }),
    initialData: [],
  });

  const { data: failedLogs = [] } = useQuery({
    queryKey: ['integration_sync_logs_failed'],
    queryFn: () => api.entidades.IntegrationSyncLog.filter({ status: 'failed' }),
    initialData: [],
  });
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="flex items-start gap-2 p-3 rounded-lg bg-blue-50 border border-blue-200 text-blue-800 text-xs">
            ...
            <p>
              ...
              <strong>preparadas en la arquitectura</strong> pero no conectadas todavía. El sistema
              opera al 100% sin ellas. Podrás activarlas cuando tengas las credenciales OAuth de
              Google Cloud.
            </p>
          </div>
      ...
      <div className="p-3 rounded-lg border border-dashed text-xs text-muted-foreground flex items-start gap-2">
        ...
        <span>
          Al cerrar cada corte de caja, el sistema crea automáticamente registros en la cola de
          sincronización con estado{' '}
          <code className="bg-muted px-1 rounded">pending_external_sync</code>. Cuando se active la
          integración real con Google, esos registros se procesarán sin pérdida de datos.
        </span>
      </div>
    </div>
  );
