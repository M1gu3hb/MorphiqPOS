...
import { normalizeCategoryName, categoryKey, ensureCategoriaExists } from '@/utils/categoriaUtils';
...
export default function CategoriasProductoSection() {
  ...
  const agregarUna = async () => {
    ...
    setSaving(true);
    try {
      const cat = await ensureCategoriaExists(api, n, { reactivate: true });
  ...
  const guardarEdicion = async () => {
    ...
    try {
      ...
      const otras = (Array.isArray(categorias) ? categorias : []).filter(
        (c) => c?.id !== editing.id,
      );
      ...
      if (estacionesActivas) {
        ...
        if (estacionId) {
          const est = (Array.isArray(estaciones) ? estaciones : []).find(
            (e) => e?.id === estacionId,
          );
      ...
      await api.entidades.CategoriaProducto.update(editing.id, payload);
  ...
  const confirmarEliminar = async () => {
    ...
    try {
      ...
      await api.entidades.CategoriaProducto.update(categoria.id, { activo: false });
      ...
      <CardContent className="space-y-5">
        <p className="text-xs text-muted-foreground">
          ...
          {estacionesActivas && (
            <>
              {' '}
              Asigna cada categoría a una <strong>estación</strong> para preparar la cocina
              filtrada.
            </>
          )}
        </p>
            ...
            {estacionesActivas && (
              <div>
                ...
                <p className="text-[10px] text-muted-foreground mt-1">
                  Si la dejas vacía, la categoría se considera en Cocina general. F1 solo guarda el
                  dato — la cocina filtrada se activará en próximas actualizaciones.
                </p>
              </div>
            )}
          </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {confirmDel?.productosCount > 0 ? (
                <>
                  Esta categoría tiene <strong>{confirmDel.productosCount}</strong> producto(s)
                  asignado(s). Reasigna esos productos a otra categoría antes de eliminarla.
                </>
              ) : (
                <>
                  La categoría <strong>"{confirmDel?.categoria?.nombre}"</strong> se marcará como
                  inactiva y dejará de aparecer en Mesero, Productos, Recetas y Portal QR. Las
                  ventas históricas no se ven afectadas.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
