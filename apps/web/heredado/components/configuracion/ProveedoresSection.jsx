...
export default function ProveedoresSection() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing?.id) {
        await api.entidades.Proveedor.update(editing.id, payload);
  ...
  const confirmarDesactivar = async () => {
    ...
    try {
      ...
      await api.entidades.Proveedor.update(confirmDel.id, { activo: false });
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <p className="text-xs text-muted-foreground">
          Administra a quién le compras insumos. Los proveedores aparecen como opción al registrar
          una compra. Desactivarlos no borra el historial — las compras pasadas siguen mostrando el
          nombre.
        </p>
