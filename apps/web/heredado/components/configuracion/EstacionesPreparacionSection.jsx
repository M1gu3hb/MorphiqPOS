...
import {
  normalizeEstacionName,
  estacionKey,
  findExistingEstacion,
  COCINA_GENERAL_NOMBRE,
  COCINA_GENERAL_COLOR,
...
export default function EstacionesPreparacionSection({ embedded = false }) {
  ...
  const guardar = async () => {
    ...
    setSaving(true);
    try {
      const todas = await api.entidades.EstacionPreparacion.list().catch(() => []);
      ...
      if (existente && existente.id !== editing?.id) {
        if (existente.activo === false) {
          ...
          await api.entidades.EstacionPreparacion.update(existente.id, {
            activo: true,
            color: form.color,
        ...
        await api.entidades.EstacionPreparacion.update(editing.id, {
          nombre,
          descripcion: form.descripcion || '',
    ...
    try {
      ...
      if (yaExiste) {
        if (yaExiste.activo === false) {
          await api.entidades.EstacionPreparacion.update(yaExiste.id, { activo: true });
        }
        invalidar();
  ...
  const confirmarDesactivar = async () => {
    ...
    try {
      ...
      await api.entidades.EstacionPreparacion.update(estacion.id, { activo: false });
  ...
  const mover = async (estacion, delta) => {
    ...
    try {
      await Promise.all([
        api.entidades.EstacionPreparacion.update(estacion.id, { orden: Number(otra.orden) || 0 }),
        api.entidades.EstacionPreparacion.update(otra.id, { orden: Number(estacion.orden) || 0 }),
      ]);
      invalidar();
    } catch (e) {
    ...
    <div className="space-y-3">
      {!embedded && (
        <p className="text-xs text-muted-foreground">
          Define las áreas de preparación de tu restaurante (barra, postres, barbacoa, etc.). Cada
          categoría de producto puede asignarse a una estación. Las categorías sin estación van a{' '}
          <strong>Cocina general</strong>.
        </p>
      )}
      ...
      {!cocinaGeneral && (
        <div className="text-xs px-3 py-2 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/60 text-amber-800 dark:text-amber-300 flex items-start gap-2">
          ...
          <div>
            ...
            categorías sin asignar. Crea Cocina general para evitar inconsistencias futuras.
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        ...
        {estacionesOrdenadas.map((e, idx) => {
          ...
          return (
            <div key={e.id} className="flex items-center gap-2 p-3 rounded-lg bg-muted/40 border">
              <span
                aria-hidden
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {confirmDel?.categoriasCount > 0 ? (
                ...
                <>
                  La estación <strong>"{confirmDel?.estacion?.nombre}"</strong> se desactivará. No
                  afecta productos, recetas, ventas ni pedidos en curso.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
