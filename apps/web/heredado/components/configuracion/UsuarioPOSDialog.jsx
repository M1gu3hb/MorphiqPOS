...
import { ROLE_LABELS_SELECTABLE, ROLES } from '@/lib/constants';
  ...
  const guardar = async () => {
    ...
    if (mostrarEstacion) {
      if (estacionesArr.length === 0) {
        toast.error(
          'Aún no hay estaciones creadas. Crea al menos una en Configuración → Operación → Estaciones de preparación.',
        );
        return;
      }
      ...
      if (!tieneEstacion && !tieneTodas) {
        toast.error(
          'Selecciona una estación para este usuario de cocina o marca "Todas las estaciones".',
        );
        return;
      }
            ...
            <Select
              ...
              onValueChange={(v) =>
                setForm({
                  ...
                  rol: v,
                  ...(v !== ROLES.KITCHEN
                    ? { estacion_preparacion_id: '', puede_ver_todas_estaciones: false }
                    : {}),
                  ...(v !== ROLES.WAITER && v !== ROLES.KITCHEN ? { color: '' } : {}),
                })
              }
            >
              <SelectTrigger>
              ...
              <SelectContent>
                {Object.entries(ROLE_LABELS_SELECTABLE).map(([k, v]) => (
            ...
            {esRolLegacyBarra && (
              <div className="mt-2 text-[11px] flex items-start gap-1.5 px-2 py-1.5 rounded-md bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/60 text-amber-800 dark:text-amber-300">
                ...
                <span>
                  ...
                  "Barra" es una <strong>estación</strong> de la cocina. Cambia el rol a{' '}
                  <strong>Cocina</strong> y asigna la estación <strong>Barra</strong>.
                </span>
              </div>
            )}
          </div>
          ...
          {mostrarEstacion && (
            <div className="rounded-xl border p-3 bg-muted/30 space-y-2">
              ...
              <p className="text-[10px] text-muted-foreground">
                Asigna a este usuario una estación específica o marca "Todas las estaciones" para
              ...
              {colorDuplicado && (
                <div className="mt-2 text-[11px] flex items-start gap-1.5 px-2 py-1.5 rounded-md bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/60 text-amber-800 dark:text-amber-300">
                  ...
                  <span>
                    ...
                    otro para evitar confusión visual.
                  </span>
                </div>
              )}
            </div>
              ...
              <div className="space-y-2 pt-1">
                <div>
                  <Label className="text-xs">
                    {isEdit ? 'Nuevo NIP (4 dígitos)' : 'NIP (4 dígitos) *'}
                  </Label>
                  <Input
                    type="password"
                    inputMode="numeric"
