        ...
        <div className="space-y-5">
          <div className="p-3 rounded-xl bg-gradient-to-r from-red-50 to-amber-50 dark:from-red-950/40 dark:to-amber-950/40 border border-red-200 dark:border-red-900 text-xs text-red-800 dark:text-red-200 flex items-start gap-2">
            ...
            <p>
              ...
              <strong>PDF blanco e imprimible</strong>, el dashboard operativo se reiniciará y el
              dinero dejado en caja será el fondo esperado para mañana.
            </p>
          </div>

          {/* === RESUMEN FINANCIERO === */}
          <Section title="Resumen financiero (sin propinas)">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <KPI
                label="Ventas reales"
                value={formatCurrency(totalGeneral)}
                icon={DollarSign}
                color={tone ? 'text-primary' : 'text-foreground'}
              />
              <KPI label="Tickets" value={String(numVentas)} icon={Receipt} />
              <KPI label="Ticket promedio" value={formatCurrency(ticketPromedio)} />
              <KPI label="Costo de ventas" value={formatCurrency(costoTotal)} />
              <KPI
            ...
            <p className="text-[10px] text-muted-foreground mt-2">
              Utilidad y margen se calculan SOLO sobre ventas reales. Las propinas no son ingreso
              del negocio.
            </p>
          </Section>
          ...
          <Section title="Propinas del día (pendientes de liquidar)">
            ...
            <p className="text-[10px] text-muted-foreground mt-2">
              Las propinas se cobran junto con la venta pero NO se liquidan en el cierre. Quedan
              registradas para entregarse a los meseros aparte.
            </p>
          </Section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                <Label className="text-xs font-semibold">
                  Dinero dejado en caja (fondo mañana)
                </Label>
                <NumericInput
                  value={dineroDejado}
            ...
            {efValido && (
              ...
              >
                ...
                <span>
                  ...
                  {tonoDiferencia === 'falta' &&
                    `Falta ${formatCurrency(Math.abs(diferencia))} en caja.`}
                </span>
              </div>
            )}
          </Section>
          ...
          <Section title="Notas y observaciones">
            ...
            <p className="text-[10px] text-muted-foreground mt-1">
              ...
              {cajaAbierta?.fecha_apertura &&
                ` Apertura: ${new Date(cajaAbierta.fecha_apertura).toLocaleString('es-MX')}`}
            </p>
          </Section>
        </div>
...
function KPI({ label, value, icon: Icon, color }) {
  return (
    <div className="p-2.5 rounded-lg bg-muted/40 border">
      ...
      <p
        className={`font-heading font-black text-base mt-0.5 truncate ${color || 'text-foreground'}`}
      >
        {value}
      </p>
    </div>
...
function FilaMetodo({ Icon, label, color, v, p, t, tone = true }) {
  return (
    <div className="grid grid-cols-4 px-3 py-2 border-t text-xs items-center">
      ...
      <span className={`text-right font-heading font-black ${color || ''}`}>
        {formatCurrency(t)}
      </span>
    </div>
  );
