...
export default function ConfiguracionQRTab({ config }) {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      portal_qr_mensaje_bienvenida:
        ...
        '¡Bienvenido! Explora nuestro menú y pídenos atención cuando quieras.',
    });
  }, [config]);
  ...
  const guardar = async () => {
    ...
    setSaving(true);
    try {
      await api.entidades.ConfiguracionNegocio.update(config.id, form);
      queryClient.invalidateQueries({ queryKey: ['config_remote'] });
    ...
    <div className="space-y-4 max-w-2xl">
      ...
      <div className="rounded-xl border border-blue-200 bg-blue-50 dark:bg-blue-950/30 dark:border-blue-800/60 p-3 text-xs text-blue-900 dark:text-blue-200 space-y-1">
        ...
        <ul className="list-disc pl-4 space-y-0.5">
          <li>
            <strong>Portal QR apagado:</strong> los QR muestran "Portal no disponible". No se puede
            ordenar ni pedir cuenta desde el celular.
          </li>
          <li>
            <strong>Portal QR encendido + "Quiero ordenar" apagado:</strong> el cliente ve el menú
            pero NO puede mandar pedidos a cocina.
          </li>
          <li>
            <strong>"Pedir cuenta" apagado:</strong> el cliente no solicita cuenta desde el QR;
            mesero/caja siguen funcionando normal.
          </li>
          <li>
            <strong>"Propina desde QR" apagada:</strong> el cliente no elige propina; mesero/caja la
            pueden aplicar.
          </li>
          <li>
            <strong>Caja, mesero y cocina siempre funcionan</strong>, esté el QR encendido o
            apagado.
          </li>
        </ul>
      </div>
      ...
      <div className="rounded-xl border bg-white p-4">
        ...
        >
          <Switch
            checked={form.portal_qr_activo}
            onCheckedChange={(v) => setForm({ ...form, portal_qr_activo: v })}
          />
        </Row>
        <Row label="Modo de menú QR" hint="Qué se muestra al cliente cuando escanea el QR.">
          <Select
            value={form.portal_qr_modo_menu}
        ...
        <Row label="Mostrar precios" hint="Si se desactiva, el menú se ve sin precios.">
          <Switch
            checked={form.portal_qr_mostrar_precios}
            onCheckedChange={(v) => setForm({ ...form, portal_qr_mostrar_precios: v })}
          />
        </Row>
        <Row label="Mostrar productos sin imagen">
          <Switch
            checked={form.portal_qr_mostrar_sin_imagen}
        ...
        return (
          <div className="rounded-xl border bg-white p-4">
            ...
            <p className="text-xs text-muted-foreground mb-2">
              ...
              ventas reales — siempre va separada al corte.
            </p>
            <Row
            ...
            >
              <Select
                value={form.portal_qr_cuenta_modo}
