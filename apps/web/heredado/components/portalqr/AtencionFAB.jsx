...
const COLORS = {
  ordenar: 'bg-advertencia text-advertencia-texto',
  cuenta: 'bg-exito text-exito-texto',
  ayuda: 'bg-peligro text-peligro-texto',
};
      ...
      <AnimatePresence>
        {showInfo && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-velo/50 z-50 flex items-end sm:items-center justify-center p-4"
          ...
          >
            <motion.div
              ...
              exit={{ y: 30, opacity: 0 }}
              className="bg-superficie rounded-2xl p-5 max-w-sm w-full shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-full bg-advertencia/15 flex items-center justify-center shrink-0">
                  <AlertCircle className="w-5 h-5 text-advertencia" />
                </div>
                <div className="flex-1">
                  <p className="font-heading font-bold text-base">Solicitud no disponible</p>
                  <p className="text-sm text-texto-sutil mt-1">{mensajeBloqueado}</p>
                </div>
              </div>
              <button
                onClick={() => setShowInfo(false)}
                className="mt-4 w-full bg-primario text-primario-texto rounded-xl py-3 font-semibold"
              >
                Entendido
              </button>
            </motion.div>
      ...
      <AnimatePresence>
        {open && !bloqueado && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-velo/40 z-40"
      ...
      <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-3">
            ...
            tiposHabilitados.map((tipo, i) => {
              ...
              return (
                <motion.button
                  ...
                  onClick={() => pedir(tipo)}
                  disabled={disabled}
                  className={`flex items-center gap-3 pl-4 pr-5 h-14 rounded-full shadow-2xl font-bold text-base ${COLORS[tipo]} active:scale-95 transition-transform disabled:opacity-50`}
        ...
        <motion.button
          ...
          className={`w-16 h-16 rounded-full shadow-2xl flex items-center justify-center disabled:opacity-50 ${
            bloqueado ? 'bg-primario/50 text-primario-texto' : 'bg-primario text-primario-texto'
          }`}
        ...
        {!open && (
          <span
            className={`text-[11px] font-semibold backdrop-blur px-2 py-0.5 rounded-full shadow ${
              bloqueado ? 'bg-advertencia/95 text-advertencia' : 'bg-superficie/90 text-texto'
            }`}
          >
