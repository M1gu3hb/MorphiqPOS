...
export default function MenuQRTab() {
  ...
  const { data: secciones = [] } = useQuery({
    ...
    queryFn: () => api.entidades.MenuQRSeccion.list('orden', 100),
    initialData: [],
  });
  ...
  const guardar = async () => {
    ...
    try {
      ...
      if (editForm.id) {
        await api.entidades.MenuQRSeccion.update(editForm.id, payload);
      } else {
        await api.entidades.MenuQRSeccion.create(payload);
      }
  ...
  const eliminar = async (s) => {
    ...
    try {
      await api.entidades.MenuQRSeccion.delete(s.id);
      queryClient.invalidateQueries({ queryKey: ['menu_qr_secciones'] });
  ...
  const subir = async (e) => {
    ...
    setUploading(true);
    try {
      const { file_url } = await api.archivos.subir({ file });
              ...
              <div>
                ...
                {editForm.imagen_url && (
                  <img
                    ...
                    alt="preview"
                    className="w-full max-h-48 object-contain bg-muted rounded mb-2"
                  />
                )}
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="flex items-end gap-2">
                  ...
                  <span className="text-xs">
                    {editForm.activo !== false ? 'Activa' : 'Inactiva'}
                  </span>
                </div>
              </div>
            </div>
