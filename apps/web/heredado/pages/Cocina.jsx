...
import { Clock, CheckCircle, Flame, ChefHat, Layers, AlertTriangle } from 'lucide-react';
...
export default function Cocina() {
  ...
  // F3.1: refetchInterval bajado de 6000 → 3000ms para sensación más viva.
  // HOTFIX 6A.3: bajado a 2000ms para que cocina quite los entregados rápido
  // cuando Mesero marca "Entregado" en su lado. Las acciones manuales
  // además usan optimistic update + refetch inmediato, así que esto es un seguro.
  const { data: pedidos, isPending: pedidosLoading } = useQuery({
    queryKey: ['pedidos_cocina', estacionesActivas ? 'all' : 'legacy'],
    queryFn: () =>
      estacionesActivas
        ? api.entidades.PedidoPreparacion.list('-created_date', 500)
        : api.entidades.PedidoPreparacion.filter({ area: 'cocina' }),
  ...
  const { data: ventas } = useQuery({
    ...
    queryFn: () => api.entidades.Venta.list('-created_date', 500),
    placeholderData: (prev) => prev,
    staleTime: 5000,
  ...
  const mesasMap = useMemo(() => {
    const m = {};
    (Array.isArray(mesas) ? mesas : []).forEach((x) => {
      if (x?.id) m[x.id] = x;
    });
    return m;
  }, [mesas]);
  ...
  const pedidosFiltrados = useMemo(() => {
    ...
    return arr;
  }, [pedidos, userScope, userEsCocinaGeneral]);
  ...
  const iniciarPedido = async (pedido) => {
    ...
    try {
      ...
      // Si falla, volvemos al estado original recargando de BD.
      queryClient.invalidateQueries({ queryKey: ['pedidos_cocina'] });
  ...
  const marcarListo = async (pedido) => {
    ...
    try {
      ...
      queryClient.refetchQueries({ queryKey: ['pedidos_cocina'], type: 'active' }).catch(() => {});
      queryClient.invalidateQueries({ queryKey: ['mesas'] });
      // F3.2: refrescar la card "Listo para recoger" del Mesero y el watcher
      // global de voz si están abiertos en otra pestaña/sesión.
          ...
          return (
            ...
            >
              ...
              >
                ...
                <h3 className="font-heading font-bold" style={{ color: colors.text }}>
