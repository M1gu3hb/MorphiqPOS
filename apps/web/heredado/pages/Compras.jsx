...
export default function Compras() {
  ...
  const { data: compras, isPending: comprasLoading } = useQuery({
    ...
    queryFn: () => api.entidades.CompraInsumo.list('-created_date', 30),
    placeholderData: (prev) => prev,
    staleTime: 5000,
  });
  ...
  const { data: gastos, isPending: gastosLoading } = useQuery({
    queryKey: ['gastos_hoy'],
    queryFn: () => api.entidades.GastoOperativo.list('-created_date', 30),
    placeholderData: (prev) => prev,
    staleTime: 5000,
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          ...
          <p className="text-sm text-muted-foreground">
            Agrega ingredientes nuevos o suma a los existentes. Se actualiza automáticamente el
            stock y los costos.
          </p>
        </button>
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground -mt-2 mb-3">
          Servicios, mantenimiento, renta, etc. NO aumentan inventario. Sí afectan la utilidad neta
          del periodo.
        </p>
        {cargandoGastos ? (
