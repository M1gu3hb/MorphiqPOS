...
| **A-49** | 8-sep-2026 | **Forma del envoltorio `comando()`** | Cinco decisiones: (1) el rol y el paquete se declaran **en cada comando**, no en un mapa central; (2) los permisos por rol son estáticos en F1.1; (3) tabla `comandos_ejecutados` para la idempotencia; (4) **sin** `emitir()` ni outbox en F1.1; (5) el resultado es una unión `{ok}`, no una excepción | (1) un mapa central es un archivo que los dos carriles editarían en cada comando nuevo: conflicto garantizado. (2) `permisos_rol` no está en las 26 tablas de F1.1-T02 y A-41 difiere la matriz a F1.5; cambiarla luego no toca ningún comando. (3) el `unique` de `ordenes` y `pagos` resuelve una orden o un pago concretos, no el caso general: la idempotencia es del envoltorio. (4) la tabla de eventos no existe en F1.1 y un `emitir` que no emite es peor que no tenerlo. (5) un `throw` obliga a un `catch` por ruta, y ahí es donde se silencian los errores que R12 prohíbe silenciar |
